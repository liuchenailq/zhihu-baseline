...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None,
            ...
            shuffle=True, draw_pictures=False, save_results=True, model_cache_path=None, early_stop=5):
        """
...
class TextDeepFM(BaseModel):
    ...
    def __init__(self,
        ...
        if self.use_dnn:
            if self.use_textcnn:
                self.dnn = DNN(
                    self.compute_input_dim(dnn_feature_columns, embedding_size) + len(filter_sizes) * num_filters * 2,
                    dnn_hidden_units,
                    activation=dnn_activation, l2_reg=l2_reg_dnn, dropout_rate=dnn_dropout, use_bn=dnn_use_bn,
                    init_std=init_std, device=device)
            else:
    ...
    def forward(self, X):
        ...
        if self.use_dnn:
            ...
            if self.use_textcnn:
                ...
                text_cnn_out = []
                for t in text_feature_list:
                    text_cnn_out.append(self.text_cnn(t).unsqueeze(1))
